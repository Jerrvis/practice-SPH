  ...
  <div class="pay-main">
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          <el-button text @click="open" class="btn">立即支付</el-button>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
import { reqOrderStatus, reqPayOrder } from "@/api";
export default {
  name: "Pay",
  setup(props) {
    ...
    let getPayInfo = async function () {
      let result = await reqPayOrder(orderId);
      console.log(result);
      if (result.code == 200) {
        console.log(result, "order");
        money.value = result.data.totalFee;
        qrcode = await QRCode.toDataURL(result.data.codeUrl);
      }
    };
    ...
    let open = function () {
      if (!timer) {
        timer = setInterval(async () => {
          let result = await reqOrderStatus(orderId);
          // if (result.code == 200) {
            clearInterval(timer);
            timer = null;
            paySuccessCode = result.code;
            ElMessage({
              type: "success",
              message: "又一个大冤种支付成功，待会就给你跳转页面",
            });
            setTimeout(() => {
              ElMessageBox.close()
      ...
      console.log(qrcode);
      ElMessageBox.confirm(`<img src="${qrcode}" alt="">`, "这是支付页面", {
        confirmButtonText: "支付完成",
        cancelButtonText: "更多支付问题",
